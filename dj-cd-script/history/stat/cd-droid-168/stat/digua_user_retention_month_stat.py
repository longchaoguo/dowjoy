...
def clearData():
    sql1 = "delete from DIGUA_USER_RETENTION_MONTH_STAT where date_format(stat_date, '%Y-%m')='"+handledate+"'"
    dbUtil_168.delete(sql1, ())

def insertMonthData():
    sql = "select count(imei) from (select imei from DIGUA_STAT_USER_LOG where date_format(created_date, '%Y-%m')='"+handledate+"' group by IMEI) T"
    logCnt = dbUtil_168.queryCount(sql, ())
    sql = "select count(imei) from DIGUA_USER where date_format(created_date, '%Y-%m')='"+handledate+"'"
    addedCnt = dbUtil_168.queryCount(sql, ())
    sql = "insert into DIGUA_USER_RETENTION_MONTH_STAT(LOGIN_CNT, ADDED_CNT, STAT_DATE) values(%s, %s, %s)"
