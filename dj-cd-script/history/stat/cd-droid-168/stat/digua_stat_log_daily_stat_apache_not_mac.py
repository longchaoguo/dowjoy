...
def statFile(dir,fileName,flag = 'False'):
    ...
    if os.path.exists(fileFullName):
        ...
        sql = "insert into DIGUA_STAT_USER_LOG_TEMP_APACHE_NOT_MAC (IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, IT, CREATED_DATE) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        dataList = []
        while 1:
...
def insertUserDailyLogs():
    sql = "select imei, count(ID) from  DIGUA_STAT_USER_LOG_TEMP_APACHE_NOT_MAC group by imei"
    rows = dbUtil_168.queryList(sql, ())
    dataList = []
    insertsql = "INSERT INTO DIGUA_STAT_USER_LOG_DAILY_APACHE_NOT_MAC (IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, IT, CREATED_DATE, PV) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    for row in rows:
        if not row:
            continue
        sql = "select IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, IT, CREATED_DATE from DIGUA_STAT_USER_LOG_TEMP_APACHE_NOT_MAC where imei=%s order by created_date limit 1"
...
def updateImeiNum():
    sql = "select id, imei, num from DIGUA_STAT_USER_LOG_DAILY_APACHE where num is null or length(num)=0"
    rows = dbUtil_168.queryList(sql, ())
    for row in rows:
        sql="select num from DIGUA_STAT_USER_LOG_TEMP_APACHE where imei=%s limit 1"
        rs=dbUtil_168.queryRow(sql, (row[1]))
        try:
            updatesql="update DIGUA_STAT_USER_LOG_DAILY_APACHE set num=%s where id=%s"
            dbUtil_168.update(updatesql, (rs[0], row[0]))
        except:
            continue
