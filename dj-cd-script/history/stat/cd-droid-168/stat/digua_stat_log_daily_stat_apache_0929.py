...
def statFile(fileName):
    ...
    if os.path.exists(fileName):
        f = open(fileName, 'rb')
        sql = "insert into DIGUA_STAT_USER_LOG_TEMP_APACHE_0929 (IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, IT, CREATED_DATE) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        dataList = []
        while True:
            line = f.readline()
            #print line
