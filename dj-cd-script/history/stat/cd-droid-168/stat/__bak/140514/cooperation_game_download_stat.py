...
dbUtil_168_droid = DBUtil('droid_stat_168')
...
def cleanData():
    ...
    dbUtil_168_droid.delete(sql, (handledate))
...
def statDowns():
    ...
    for row in rows:
        ...
        if len(datalist) > 1000:
            insertData(dbUtil_168_droid, insertsql, datalist)
            datalist = []
    if datalist:
        insertData(dbUtil_168_droid, insertsql, datalist)
        datalist = []
...
def statDownsSum():
    ...
    downs = dbUtil_168_droid.queryCount(sql, (handledate))
    ...
    rows = dbUtil_168_droid.queryList(sql, (handledate))
    for row in rows:
    ...
    dbUtil_168_droid.insert(sql, (downs, detailDowns, handledate))
...
if __name__ == "__main__":
    try:
        ...
        if dbUtil_168_droid: dbUtil_168_droid.close()
