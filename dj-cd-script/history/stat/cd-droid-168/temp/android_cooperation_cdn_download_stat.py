...
domain = "res3.d.cn"
domain_tencent = "u.androidgame-store.com"
domain_360 = "u.androidgame-store.com"
domain_360_2 = "u.androidgame-store.com"
domain_baidu = "u.androidgame-store.com"
domain_tencent_2 = "u.androidgame-store.com"
domain_wap = "p.androidgame-store.com"
domain_web = "down.androidgame-store.com"
logDir = '/home/downjoy/stat/ftp/down/addownlog/'
logDirCDN = '/home/downjoy/stat/ftp/down/addownlog/cdn/'
...
fileNameCDN_wap = domain_wap + '_%s.gz'
fileNameCDN_web = domain_web + '_%s.gz'
...
pattern4 = re.compile("(?P<IP>\S*) \S* \S* \[(?P<TIME>\S+) \S+\] \S+ http://down.androidgame-store.com\/\w+\/\w+\/(?P<GAME>\w+)\/\d+\/(?P<RID>\d+)\/(?P<PKG>\S*)\?f=(?P<PARAM>[^&=]*)\S* \S* (?P<STATUS>\d+) \S* \S* \"(?P<UA>[^\"]*)\"")
pattern5 = re.compile("(?P<IP>\S*) \S* \S* \[(?P<TIME>\S+) \S+\] \S+ http://p.androidgame-store.com\/\w+\/\w+\/(?P<GAME>\w+)\/\d+\/(?P<RID>\d+)\/(?P<PKG>\S*)\?f=(?P<PARAM>[^&=]*)\S* \S* (?P<STATUS>\d+) \S* \S* \"(?P<UA>[^\"]*)\"")
#有版权
...
def statFile(fileName, pattern , chanFlag, removeUtil1, removeUtil2):
    ...
    while True:
        ...
        if not chanFlagValue:
            continue
        if chanFlagValue in [41, 51, 61, 71, 81, 91, 101, 111, 121, 131]:
        ...
        if len(tempDataList) >= 1000:
            insertDataToLog(dbUtil_187, tempDataList)
            tempDataList = []
    if tempDataList:
        insertDataToLog(dbUtil_187, tempDataList)
        tempDataList = []
...
def deleteDBData():
    ...
    sql1 = "delete from ANDROID_GAME_DOWNLOAD_LOG where CHANNEL_FLAG in (40,41,50,51,60,61,70,71,80,81,90,91,100,101,110,111,120,121,130,131,140) and CREATED_DATE >= %s and CREATED_DATE < %s"
    dbUtil_187.delete(sql1, (yesterdayStr, nextDate))

def handleCDNLog(domain, gzFile, pattern, user, passwd): #cdn日志统计
    if not os.path.exists(gzFile) and not os.path.exists(gzFile[:gzFile.find('.gz')]):
        getCDNFile(domain, gzFile, user, passwd)
    if os.path.exists(gzFile) and not os.path.exists(gzFile[:gzFile.find('.gz')]):
...
if __name__ == '__main__':
    ...
    try:
        ...
        #handleCDNLog(domain_tencent, logDirCDN + fileNameCDN_tencent % yesterdayStampCDN, pattern3, 'cdnpush', 'cdn123!@#push') # 腾讯CDN统计       
        handleCDNLog(domain_web, logDirCDN + fileNameCDN_web % yesterdayStampCDN, pattern4, 'cdnpush', 'cdn123!@#push') # web统计       
        handleCDNLog(domain_wap, logDirCDN + fileNameCDN_wap % yesterdayStampCDN, pattern5, 'cdnpush', 'cdn123!@#push') # wap统计       
        #handleCDNLog(domain_360, logDirCDN + fileNameCDN_360 % yesterdayStampCDN, pattern3, 'cdnpush', 'cdn123!@#push')
        #andleCDNLog(domain_360_2, logDirCDN + fileNameCDN_360_2 % yesterdayStampCDN, pattern3, 'cdnpush', 'cdn123!@#push')
        #andleCDNLog(domain_baidu, logDirCDN + fileNameCDN_baidu % yesterdayStampCDN, pattern3, 'cdnpush', 'cdn123!@#push')
        #andleCDNLog(domain_tencent_2, logDirCDN + fileNameCDN_tencent_2 % yesterdayStampCDN, pattern3, 'cdnpush', 'cdn123!@#push')
        #统计多特等ftp日志数据
        #statFtpFile()
    except Exception, ex:
