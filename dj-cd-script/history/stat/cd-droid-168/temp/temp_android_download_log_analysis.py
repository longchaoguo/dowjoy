...
reportReceivers = ["qicheng.meng@downjoy.com"]
...
DOMAIN_CDN_CARRIER = {
                      ...
                      "p.androidgame-store.com":   {"carrier":CARRIER_DNION,"username":"cdnpush","password":"cdn123!@#push","channel":[20],"pattern":   re.compile("^(?P<IP>\S*) +\S* +\S* +\[(?P<TIME>\S+) +\S+\] +\S+ +(http://p.androidgame-store.com)?(:80)?(/android)?/+new/+(?P<GAME>\w+)/+\d+/(?P<GID>\d+)/(?P<FILE_NAME>\S+\.(apk|dpk|zip))(\?(f=(?P<CHANNEL_FLAG>\w*))?)?.* +\S+\" +(?P<STATUS>\d+) +\S* +\"(?P<SOURCE>[^\"]*)\" +\"(?P<UA>[^\"]*)\".*$")},
...
mailTo = ['lin.he@downjoy.com','qicheng.meng@downjoy.com']
