...
def handleFtpFile():
    sql="select SRC_DIR, SRC_FILE, LOCAL_DIR, LOCAL_FILE, IP, PORT, IS_DELETE_SRC_FILE, IS_DELETE_LOCAL_FILE, CREATED_DATE from FTP_LOG_CONFIG where id in (73,74,75,118) order by ID;"
    rows = dbUtil_168.queryList(sql, ())
    for row in rows:
        if not row:
            continue
...
def clearData():
    ...
    sql1 = "truncate DIGUA_STAT_USER_LOG_DAILY_TEMP"
    dbUtil_168.truncate(sql1, ())

def insertUserDailyLogs():
    #sql1 = "truncate DIGUA_STAT_USER_LOG_DAILY"
    #dbUtil_168.truncate(sql1, ())
    #sql = "select T.imei, T.IP, T.RESOLUTION_ID, ifnull(T.RESOLUTION_NAME, ''), T.OS_ID, ifnull(T.OS_NAME, ''), ifnull(T.version, ''), T.CLIENT_CHANNEL_ID, ifnull(T.CLIENT_CHANNEL_NAME, ''), ifnull(T.DEVICE, ''), ifnull(T.num, ''), T.CREATED_DATE, SUM(PV) from (select IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV from DIGUA_STAT_USER_LOG_DAILY_TEMP order by created_date) T group by T.imei"
    #sql = "select imei, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE,count(id) from DIGUA_STAT_USER_LOG_TEMP group by imei"
    sql = "select imei, count(ID) from  DIGUA_STAT_USER_LOG_TEMP_"+str(handledate)+" group by imei"
    rows = dbUtil_168.queryList(sql, ())
    dataList = []
    insertsql = "INSERT INTO DIGUA_STAT_USER_LOG_DAILY_"+str(handledate)+" (IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, it, CREATED_DATE, PV) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    for row in rows:
        if not row:
            continue
        sql = "select IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, IT, CREATED_DATE from DIGUA_STAT_USER_LOG_TEMP_"+str(handledate)+" where imei=%s order by created_date limit 1"
...
#        insertUserDailyLogs()
        ...
        statFile("/opt/logs/100463_14.txt")
    except Exception, ex:
