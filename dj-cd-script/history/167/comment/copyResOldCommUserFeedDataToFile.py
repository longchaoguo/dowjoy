...
def main():
    ...
    list = []
    for out in db.comm_user_feed.find({"favorites.id":resId, "favorites.resType":1},{"_id":1, "favorites":1}).batch_size(10000):
        tmp = {}
        tmp["uid"] = out["_id"]
        favorites = out["favorites"]
        for favorite in favorites:
            if(favorite["id"]==resId and favorite["resType"]==resType):
                tmp["favorites"] = favorite
                break;
        list.append(tmp)
