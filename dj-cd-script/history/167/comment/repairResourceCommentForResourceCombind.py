...
mailSubject = u"repairResourceCommentForResourceCombind.py".encode("gbk")
...
def main():
    ...
    print combindIds
    for out in db.resourceComment.find({},{"resourceKey":1, "replyId":1, "subComments":1, "subs":1}).batch_size(10000):
        if out.has_key("resourceKey"):
            ...
            if resType==1 and combindIds.has_key(resId):
                ...
                db.resourceComment.update({"_id":id}, {"$set":setValue})
                incValue = {"commentCount":1}
                if not out.has_key("replyId"):
                    incValue["firstCommentCount"]=1 
                db.resourceInfo.update({"_id":newResourceKey}, {"$inc":incValue})         
