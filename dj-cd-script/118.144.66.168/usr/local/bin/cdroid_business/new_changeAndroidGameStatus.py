...
MONITOR_MAIL=['shan.liang@downjoy.com','lin.he@downjoy.com']
...
#记录需要推送给cdn的url,仅将内容推送给指向该节点的cdn域名而不是所有cdn域名
...
def notifyCDN():
    ...
    for url, vendor in NeedUpdateUrls.items():
        ...
        if "dilian" == vendor:
            ...
            notifyUrl_gosun.append(url)
    if len(dunionUrls) > 1 :
        dunionUrls = dunionUrls[1:]
        notifyDunionCDN(dunionUrls)
    if len(notifyUrl_gosun) > 0 :
...
def notifyGosunCDN(urlList):
    ...
    try:
        ...
        httpConn.close()
        print "notice gosun cdn finish"
    except Exception,ex:
        print "notice gosun cdn fail"
        print "+++++++++%s"%ex
    finally:
        if httpConn:
            httpConn.close()
...
def changeSyncStatus():
    ...
    postUrls=""
    #查询没有同步完成的非广告包
    sql='''select P.URL, P.FILE_SIZE, P.ID, P.GAME_ID, P.CREATED_DATE,P.REPLACE_PKG_ID,
    P.MD5,P.IS_DEFAULT from GAME_PKG P inner join GAME G on G.ID = P.GAME_ID 
    where P.SYNC_STATUS = 0 and (P.ORIGINAL_ID IS NULL or P.ORIGINAL_ID='') and 
    G.COPYRIGHT_TYPE = %s order by ID limit 20'''%(COPYRIGHT_TYPE)
    cursor.execute(sql)
    rs1 = cursor.fetchall()  
    #print len(rs1)
