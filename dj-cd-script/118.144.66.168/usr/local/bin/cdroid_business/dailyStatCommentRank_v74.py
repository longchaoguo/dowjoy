...
# connection = pymongo.Connection('192.168.9.25',27017)
...
def main():
    ...
    #游戏评论排行
    statResourceCommentRankingDaily()

# ·每日精彩评论
#     ·每天点赞量倒序，30条？
#     ·提供删除接口
def statGoodCommentRankingDaily():
    ...
    for out in db.resourceComment.find({"opTime":{"$gte":yestodayMilliSeconds, "$lte":todayMilliSeconds}, "goodRat":{"$gt":0}}).sort("goodRat",pymongo.DESCENDING):
        try:
            ...
            if(filterKeyWord(out["content"])!=True and out["rname"] != None and out["rname"] != ""):
                out["resCommentCount"] = getResourceCommentCount(out["resourceKey"])
...
#1435815588084
#4210473
def statCommentExpertRankingDaily():
    statCount = 10 #要求量
    doStat = False
...
#                     print commentId
                ...
                try: 
                    cnt = 0
                    cnt = db.resourceComment.find({"replyId":commentId}).count()
                    replyCount = replyCount + cnt
                except Exception:
                    error = 0
#                     print commentId
            userObj["user"] = userId
...
# ·游戏评论排行
#     ·每天新增评论最多应用倒序，10条？
def statResourceCommentRankingDaily():
    statCount = 10 #要求量
    resourceRankMap = {}
    ...
    loop = 0
    for item in sorted(resourceRankMap.items(), lambda x, y: cmp(x[1], y[1]), reverse=True) :
        dbObj = {}
        dbObj["_id"] = ObjectId()
        commentCount = resourceRankMap.get(item[0])
        dbObj["resourceKey"] = item[0]
        dbObj["commentCount"] = commentCount
        dbObj["statTime"] = intToday
        dbObj["totalCommentCount"] = getResourceCommentCount(item[0])
...
if __name__ == '__main__':
    try:
        print "dailyStatCommentRank_v74.py start time: %s"%(datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d %H:%M:%S"))
        main()
        print "dailyStatCommentRank_v74.py end time: %s"%(datetime.datetime.strftime(datetime.datetime.now(), "%Y-%m-%d %H:%M:%S"))
    except Exception, ex:
        fp = StringIO.StringIO()    #创建内存文件对象
        traceback.print_exc(file = fp)
        ERROR_MSG = str(fp.getvalue())
