...
handledate = None
mailToUsers = ["christina.gu@downjoy.com", "jacky@downjoy.com", "chengbao.yang@downjoy.com", "leepong@downjoy.com","yichuan.dai@downjoy.com","guoqiang.sun@downjoy.com"]
...
def handleDigua():
    sql="select SRC_DIR, SRC_FILE, LOCAL_DIR, LOCAL_FILE, IP, PORT, IS_DELETE_SRC_FILE, IS_DELETE_LOCAL_FILE, CREATED_DATE from FTP_LOG_CONFIG where id in(161,162,163,164,205,225) order by ID;"
    rows = dbUtil_168.queryList(sql, ())
    for row in rows:
        if not row:
            continue
...
def writeFile(reportFile):
    sheet1 = workbook.add_sheet(u'web关键字Top500', cell_overwrite_ok=True)
    sheet1.write(0,0, u'关键字')
    sheet1.write(0,1, u'数量')
    sheet2 = workbook.add_sheet(u'digua关键字Top500', cell_overwrite_ok=True)
...
def getWebTop100():
    sql="select CACHE_KEY, COUNT(ID) AS CNT FROM CACHE_KEYS_WEB group BY CACHE_KEY ORDER BY CNT DESC LIMIT 500"
    return dbUtil_168.queryList(sql,())

def getDiguaTop100():
    sql="select CACHE_KEY, COUNT(ID) AS CNT FROM CACHE_KEYS_DIGUA group BY CACHE_KEY ORDER BY CNT DESC LIMIT 500"
    return dbUtil_168.queryList(sql,())


#发送邮件
def sendmail(reportFile):
    body = "您好：<br>搜索关键字top500统计报表，见附件<br>如有需求和问题，请和孙国强联系。"
    ...
    main_msg['Subject'] = u"搜索关键字TOP500_%s.xls"%(handledate)
...
if __name__ == '__main__':
    ...
    #handleWeb()
    #handleDigua()
    reportFile = fileDir + "关键字TOP500_%s.xls"%(handledate)
