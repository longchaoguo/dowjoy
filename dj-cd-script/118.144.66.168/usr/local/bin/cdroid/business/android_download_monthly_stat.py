...
#####################################################
#此脚本每天执行一次，将安卓月下载量top300更新至旧top300表里。
#####################################################
import datetime
import StringIO
import traceback
...
mailSubject=u"安卓月下载量统计错误信息".encode("gbk")
mailTo=['shan.liang@downjoy.com']
mailContents=u'Hi: \n'

#每天将过去30天内的下载量top300更新至旧top300表里
def updateMonthlyData(type):
    table = ""
    if type == 1:#游戏
        table = "DOWNLOAD_MONTHLY_GAME_TOP300"
    else:
        return
    ...
    #查询新的月下载量排行
    sql = "select GAME_ID, sum(DOWNS) as CNT from ANDROID_GAME_DOWNLOAD_DAILY where GAME_ID>0 and datediff(%s, CREATED_DATE)>0 and datediff(%s, CREATED_DATE)<=30 and RESOURCE_TYPE=%s group by GAME_ID order by CNT desc limit 300"
    rows = dbUtil_download_stat.queryList(sql, (handledate, handledate, type))
    if not rows:
        return
    for row in rows:
...
def sendMail():
    global mailContents
    mailContents=(mailContents+u'月下载量统计脚本执行出错，\n错误信息：%s\n谢谢'%(ERROR_MSG)).encode('gbk')
