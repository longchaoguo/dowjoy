...
def clearRepeatedNumData():
    sql = "SELECT num, COUNT(imei) AS cnt FROM (SELECT num, imei FROM DIGUA_STAT_USER_LOG_DAILY WHERE LENGTH(num)>0 UNION ALL SELECT num, imei FROM DIGUA_USER WHERE LENGTH(num)>0 AND DATEDIFF(%s, created_date)<367)T GROUP BY num having cnt>3"
    rows = dbUtil_168.queryList(sql, (handledate))
    for row in rows:
        try:
            sql = "delete from DIGUA_STAT_USER_LOG_DAILY where num=%s order by created_date desc limit %s"
...
def insertUserLog():
    sql = "insert into DIGUA_STAT_USER_LOG(IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV) select IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV from DIGUA_STAT_USER_LOG_DAILY"
    dbUtil_168.insert(sql, ())

def insertNewUser():
    sql = "SELECT IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE FROM DIGUA_STAT_USER_LOG_DAILY WHERE IMEI NOT IN (SELECT IMEI FROM DIGUA_USER where DATEDIFF(created_date, %s)<0)"
    rows = dbUtil_168.queryList(sql, (handledate))
...
def initChannelRetentionCnt():
    sql = "select U.client_channel_id, count(*) from DIGUA_USER U inner join DIGUA_STAT_USER_LOG L on U.IMEI=L.IMEI where datediff(%s, U.created_date)=0 and datediff(%s, L.created_date)=0 group by U.client_channel_id"
...
def insertClientChannelUser():
    sql = "SELECT U.client_channel_id, U.client_channel_name, COUNT(U.imei), SUM(L.pv), L.created_date FROM DIGUA_STAT_USER_LOG L inner join DIGUA_USER U on U.imei=L.imei where datediff(%s, L.created_date)=0 GROUP BY U.client_channel_id"
    rows = dbUtil_168.queryList(sql, (handledate))
...
def getUserLogCnt():
    sql = "select count(imei) from DIGUA_STAT_USER_LOG_DAILY"
...
def getUserDayCnt(datetimeStr):
    ...
    sql = "select count(U.imei) from (select imei from DIGUA_USER where datediff(%s,  created_date)=0) U inner join DIGUA_STAT_USER_LOG_DAILY L on U.imei=L.imei"
...
def getUserPv():
    sql = "select sum(pv) from DIGUA_STAT_USER_LOG_DAILY"
    return dbUtil_168.queryCount(sql, ())
...
def updateOldUserData():
    ...
    #sql="insert DIGUA_STAT_DAILY_CLIENT_CHANNEL_CNT(CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, CNT, STAT_DATE) select CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, count(ID) as CNT, CREATED_DATE from DIGUA_STAT_USER_LOG where datediff(%s, created_date)=0 group by CLIENT_CHANNEL_ID order by CLIENT_CHANNEL_ID;"
    dbUtil_168.insert(sql, (handledate))
    initClientChannelRate()
    #sql="select count(*), CLIENT_CHANNEL_ID, max(CLIENT_CHANNEL_NAME), max(), max(CLIENT_CHANNEL_ID), max(INSTALL_TYPE), convert(varchar(100), max(STAT_DATE), 23) from CLIENT_STAT_DAILY_UNIQUE_ADDED_USER where datediff(day, STAT_DATE , '%s')=0 group by CLIENT_CHANNEL_ID, VERSION, INSTALL_TYPE"%(handledate)
    ...
    #sql="SELECT COUNT(imei), client_channel_id, client_channel_name, VERSION, 2, created_date FROM  DIGUA_USER where datediff(%s, created_date)=0 GROUP BY client_channel_id, VERSION"
    #print sql
    #print handledate
    rows = dbUtil_168.queryList(sql, (handledate))
    for row in rows:
