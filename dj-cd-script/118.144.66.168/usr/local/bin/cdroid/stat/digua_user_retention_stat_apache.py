...
def clearData():
    ...
    sql = "delete from DIGUA_USER_APACHE where datediff(%s,CREATED_DATE) = 0"
    dbUtil_168.delete(sql, (handledate))
    sql = "delete from DIGUA_USER_RETENTION_STAT_APACHE where datediff(%s,stat_date) = 0"
    dbUtil_168.delete(sql, (handledate))
    sql = "truncate table DIGUA_USER_NUM_BLACKLIST_APACHE"
    dbUtil_168.delete(sql, ())
    sql = "truncate table DIGUA_STAT_NEW_ADDED_LOG_APACHE"
    dbUtil_168.delete(sql, ())
    sql = "delete from DIGUA_CLIENT_CHANNEL_USER_RETENTION_STAT_APACHE where datediff(%s,stat_date) = 0"
    dbUtil_168.delete(sql, (handledate))

def clearRepeatedNumData():
    sql = "insert into DIGUA_STAT_NEW_ADDED_LOG_APACHE (IMEI, NUM) SELECT IMEI, NUM FROM DIGUA_STAT_USER_LOG_DAILY_APACHE WHERE IMEI NOT IN (SELECT IMEI FROM DIGUA_USER_APACHE where DATEDIFF(created_date, %s)<0)"
    rows = dbUtil_168.insert(sql, (handledate))

    sql = "SELECT num, COUNT(imei) AS cnt FROM (SELECT num, imei FROM DIGUA_STAT_NEW_ADDED_LOG_APACHE WHERE LENGTH(num)>0 UNION ALL SELECT num, imei FROM DIGUA_USER_APACHE WHERE LENGTH(num)>0 AND DATEDIFF(%s, created_date)<367)T GROUP BY num having cnt>3"
    rows = dbUtil_168.queryList(sql, (handledate))
    for row in rows:
        dbUtil_168.insert("insert into DIGUA_USER_NUM_BLACKLIST_APACHE(IMEI, NUM) select IMEI, NUM FROM DIGUA_STAT_NEW_ADDED_LOG_APACHE WHERE NUM=%s", (row[0]))
...
def insertUserLog():
    sql = "insert into DIGUA_STAT_USER_LOG_APACHE(IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV) select IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV from DIGUA_STAT_USER_LOG_DAILY_APACHE"
    dbUtil_168.insert(sql, ())

def insertNewUser():
    sql = "SELECT IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE FROM DIGUA_STAT_USER_LOG_DAILY_APACHE WHERE IMEI NOT IN (SELECT IMEI FROM DIGUA_USER_APACHE where DATEDIFF(created_date, %s)<0)"
    rows = dbUtil_168.queryList(sql, (handledate))
    insertsql = "insert into DIGUA_USER_APACHE(id, IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    dataList = []
    id = dbUtil_168.queryCount("select id from DIGUA_USER_APACHE order by id desc limit 1", ())
    for row in rows:
        if not row:
            continue
        #try:
...
def initChannelRetentionCnt():
    sql = '''SELECT U.client_channel_id, COUNT(U.imei) 
            FROM DIGUA_USER_APACHE U
            WHERE DATEDIFF(%s,  created_date)=0 
            AND EXISTS(SELECT imei FROM DIGUA_STAT_USER_LOG_DAILY_APACHE L WHERE U.imei = L.imei)
...
def initChannelRetentionRate():
    sql = "select CLIENT_CHANNEL_ID, added_cnt from DIGUA_CLIENT_CHANNEL_USER_RETENTION_STAT_APACHE where datediff(%s, stat_date)= 0"
...
def insertClientChannelUserRetention():
    sql = '''SELECT client_channel_id, client_channel_name, COUNT(imei), SUM(pv), created_date 
            FROM DIGUA_STAT_USER_LOG_DAILY_APACHE 
            GROUP BY client_channel_id
          '''
    rows = dbUtil_168.queryList(sql)
    insertsql = "insert into DIGUA_CLIENT_CHANNEL_USER_RETENTION_STAT_APACHE(CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, LOG_CNT, PV, STAT_DATE, ADDED_CNT, 1DAY_CNT, 7DAY_CNT, 30DAY_CNT, 1DAY_RATE, 7DAY_RATE, 30DAY_RATE) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    dataList = []
    for row in rows:
        if not row:
            continue
...
def insertUserRetention():
    ...
    pv = getUserPv()
    sql = "insert into DIGUA_USER_RETENTION_STAT_APACHE(LOG_CNT, ADDED_CNT, 1DAY_CNT, 7DAY_CNT, 30DAY_CNT, 1DAY_RATE, 7DAY_RATE, 30DAY_RATE, PV, STAT_DATE) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    dbUtil_168.insert(sql, (logCnt, addedCnt, dayCnt1, dayCnt7, dayCnt30, dayRate1, dayRate7, dayRate30, pv, handledate))

def getUserLogCnt():
    sql = "select count(imei) from DIGUA_STAT_USER_LOG_DAILY_APACHE"
...
def getUserDayCnt(datetimeStr):
    ...
    sql = "select count(U.imei) from (select imei from DIGUA_USER_APACHE where datediff(%s,  created_date)=0) U inner join DIGUA_STAT_USER_LOG_DAILY_APACHE L on U.imei=L.imei"
    return dbUtil_168.queryCount(sql, (datetimeStr))

def getUserRetentionRate(datetiemStr, dayCnt):
    sql = "select added_cnt from DIGUA_USER_RETENTION_STAT_APACHE where datediff(%s, stat_date)=0"
...
def getUserPv():
    sql = "select sum(pv) from DIGUA_STAT_USER_LOG_DAILY_APACHE"
    return dbUtil_168.queryCount(sql, ())
...
def clearTempTableData():
    ...
    sql1 = "truncate DIGUA_STAT_USER_LOG_DAILY_APACHE"
    dbUtil_168.truncate(sql1, ())
...
def updateOldUserData():
    ...
    dbUtil_168.delete(sql, (handledate))
    #sql = "insert into DIGUA_STAT_USER_LOG_DAILY_APACHE (IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV) select IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV from DIGUA_STAT_USER_LOG_APACHE where datediff(%s, created_date)=0"
    #dbUtil_168.insert(sql, (handledate))
    sql="insert DIGUA_STAT_DAILY_CLIENT_CHANNEL_CNT(CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, CNT, STAT_DATE) select CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, count(ID) as CNT, CREATED_DATE from DIGUA_STAT_USER_LOG_DAILY_APACHE group by CLIENT_CHANNEL_ID order by CLIENT_CHANNEL_ID;"
    dbUtil_168.insert(sql, ())
    initClientChannelRate()
    sql ="SELECT COUNT(U.imei), U.client_channel_id, U.client_channel_name, L.VERSION, L.IT, L.created_date FROM DIGUA_STAT_USER_LOG_DAILY_APACHE L inner join DIGUA_USER_APACHE U on U.imei=L.imei where datediff(%s, U.created_date)=0 GROUP BY U.client_channel_id, L.VERSION, L.IT"
    rows = dbUtil_168.queryList(sql, (handledate))
    for row in rows:
        cnt=row[0]
...
if __name__ == '__main__':
    ...
    try:
        ...
        #获取渠道用户留存率，并入表DIGUA_CLIENT_CHANNEL_USER_RETENTION_STAT
