...
def clearRepeatedNumData():
    sql = "SELECT num, COUNT(imei) AS cnt FROM (SELECT num, imei FROM DIGUA_STAT_USER_LOG_DAILY_APACHE WHERE LENGTH(num)>0 UNION ALL SELECT num, imei FROM DIGUA_USER_APACHE WHERE LENGTH(num)>0 AND DATEDIFF(%s, created_date)<367)T GROUP BY num having cnt>3"
    rows = dbUtil_168.queryList(sql, (handledate))
    for row in rows:
        try:
            sql = "delete from DIGUA_STAT_USER_LOG_DAILY_APACHE where num=%s order by created_date desc limit %s"
...
def insertUserLog():
    sql = "insert into DIGUA_STAT_USER_LOG_APACHE(IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV) select IMEI, IP, RESOLUTION_ID, RESOLUTION_NAME, OS_ID, OS_NAME, VERSION, CLIENT_CHANNEL_ID, CLIENT_CHANNEL_NAME, DEVICE, NUM, CREATED_DATE, PV from DIGUA_STAT_USER_LOG_DAILY"
...
def initChannelRetentionCnt():
    sql = "select U.client_channel_id, count(*) from DIGUA_USER_APACHE U inner join DIGUA_STAT_USER_LOG_APACHE L on U.IMEI=L.IMEI where datediff(%s, U.created_date)=0 and datediff(%s, L.created_date)=0 group by U.client_channel_id"
    ...
    sql = "select U.client_channel_id, count(*) from DIGUA_USER_APACHE U inner join DIGUA_STAT_USER_LOG_APACHE L on U.IMEI=L.IMEI where datediff(%s, U.created_date)=0 and datediff(%s, L.created_date)=0 group by U.client_channel_id"
    rows = dbUtil_168.queryList(sql, (datetime1, handledate))
    for row in rows:
        if not row:
            continue
        CHANNEL_RETENTIONS_CNT_1DAY[str(row[0])]=row[1]
    sql = "select U.client_channel_id, count(*) from DIGUA_USER_APACHE U inner join DIGUA_STAT_USER_LOG_APACHE L on U.IMEI=L.IMEI where datediff(%s, U.created_date)=0 and datediff(%s, L.created_date)=0 group by U.client_channel_id"
    rows = dbUtil_168.queryList(sql, (datetime7, handledate))
    for row in rows:
        if not row:
            continue
        CHANNEL_RETENTIONS_CNT_7DAY[str(row[0])]=row[1]
    sql = "select U.client_channel_id, count(*) from DIGUA_USER_APACHE U inner join DIGUA_STAT_USER_LOG_APACHE L on U.IMEI=L.IMEI where datediff(%s, U.created_date)=0 and datediff(%s, L.created_date)=0 group by U.client_channel_id"
    rows = dbUtil_168.queryList(sql, (datetime30, handledate))
    for row in rows:
        if not row:
            continue
...
def insertClientChannelUser():
    sql = "SELECT U.client_channel_id, U.client_channel_name, COUNT(U.imei), SUM(L.pv), L.created_date FROM DIGUA_STAT_USER_LOG_APACHE L inner join DIGUA_USER_APACHE U on U.imei=L.imei where datediff(%s, L.created_date)=0 GROUP BY U.client_channel_id"
    rows = dbUtil_168.queryList(sql, (handledate))
